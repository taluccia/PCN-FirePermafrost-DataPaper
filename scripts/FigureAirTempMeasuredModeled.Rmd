...
siteData %>% 
  ...
  mutate(residualError = `Measured TempC` - `Modelled TempC`) %>% 
    mutate(relativeError = (`Measured TempC` - `Modelled TempC`) / `Modelled TempC` * 100) %>%
  summarise(residualErrorMean = mean(residualError),
            relativeErrorMean = mean(relativeError))
```
